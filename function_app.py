...
@app.timer_trigger(schedule=DUPLICATE_TOKEN_CRON, arg_name="myTimer", run_on_startup=True, use_monitor=False)
def DuplicateTokens(myTimer: func.TimerRequest) -> None:
    tokens.run_clear_duplicate_tokens()

@app.timer_trigger(schedule=REDUNDANT_TOKEN_CRON, arg_name="myTimer", run_on_startup=True, use_monitor=False)
