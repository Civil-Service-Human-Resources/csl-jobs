...
describe('Test clear redundant tokens', () => {
  ...
  it('should delete non-user invalid tokens', async () => {
    ...
    expect(result.text).to.eq('Deleted 2 invalid non-user tokens and 0 invalid user tokens. Valid user tokens remaining: 0. Valid non-user tokens remaining: 0.')
  })
  it('should delete user invalid tokens', async () => {
    ...
    expect(result.text).to.eq('Deleted 0 invalid non-user tokens and 1 invalid user tokens. Valid user tokens remaining: 0. Valid non-user tokens remaining: 0.')
  })
  it('shouldnt delete any tokens if the counts are 0', async () => {
    ...
    expect(result.text).to.eq('Deleted 0 invalid non-user tokens and 0 invalid user tokens. Valid user tokens remaining: 0. Valid non-user tokens remaining: 0.')
  })
})
