...
describe('Test clear redundant tokens', () => {
  ...
  it('should delete non-user invalid tokens', async () => {
    ...
    expect(result.text).to.eq('Deleted 2 invalid non-user tokens and 0 valid user tokens')
  })
  it('should delete user invalid tokens', async () => {
    ...
    expect(result.text).to.eq('Deleted 0 invalid non-user tokens and 1 valid user tokens')
  })
  it('shouldnt delete any tokens if the counts are 0', async () => {
    ...
    expect(result.text).to.eq('Deleted 0 invalid non-user tokens and 0 valid user tokens')
  })
})
